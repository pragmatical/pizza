...
class Component(BaseSolutionComponent[ExperimentDataModel]):
    ...
    def execute(self, data_model: ExperimentDataModel) -> ExperimentDataModel:
        ...
        expected_embeddings = [
            self.call_embedding_function(str(e), self.embedding_model) for e in data_model.request.expected_output
        ]
        completion_embeddings = [
            self.call_embedding_function(str(c), self.embedding_model) for c in data_model.model_output.completions
        ]
